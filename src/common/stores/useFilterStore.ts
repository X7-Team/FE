...
interface FilterState {
  ...
  getQuery: (prefix?: string) => IParams;
  resetFilter: (prefix?: string) => void;
  resetFilterExceptPageAndLimit: (prefix?: string) => void;
  updateQueryParams: (params: IParams, prefix?: string) => void;
  onChangeSearchInput: (
    text: string,
    options: { enableOnChangeSearch: boolean; prefix?: string },
  ) => void;
}
...
export const useFilterStore = create<FilterState>((set, get) => ({
  ...
  updateQueryParams: (params: IParams, prefix = "") => {
    const { query } = get();
